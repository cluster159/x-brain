...
def detect_neuron(inputx, inputy, inputz,detect_list,brain_dictionary,cube_size): #version for input is a single point; datapoints is an OrderedDict
            ...
            if (x,y,z) in brain_dictionary[(x_index,y_index,z_index)]:
                for k in brain_dictionary[(x_index,y_index,z_index)][(x,y,z)]:
                    if ((k[0],k[1])) not in involved_neurons:
                        involved_neurons[(k[0],k[1])] = i[3]
                    else:
                        if involved_neurons[(k[0],k[1])] < i[3]:
                            involved_neurons[(k[0],k[1])] = i[3]
...
def inference(inputpoints,detect_list,brain_dictionary,cube_size): #assume inputpoints i s a list of lists/tuples; datapoints is an OrderedDict，這個沒包含vector
    ...
    for i in xrange(len(inputpoints)):
        involved_points = detect_neuron(inputx=inputpoints[i][0],inputy=inputpoints[i][1],inputz=inputpoints[i][2],detect_list=detect_list,brain_dictionary=brain_dictionary,cube_size=cube_size)
