...
# fig = plt.figure()
maxp = bound.maxp
minp = bound.minp #sigma=3,取2個sigma
...
def add_neuro_space(mark_x, xindex, mark_y, yindex,mark_z, zindex):
    ...
    try:
        with open("xbrain_space_pickle_100_0p3_20000/"+neuro_space_name) as fp:
    ...
    for i in detect_list:
        ...
        if (mark_x, xindex, mark_y, yindex,mark_z, zindex) not in neuro_space_list:
            ...
            for kk in neuro_space_list:
                if abs(kk[1]*kk[0] - xindex*mark_x) > 7 or abs(kk[2]*kk[3] - mark_y*yindex) > 7 or abs(kk[4]*kk[5] - mark_z*zindex) > 7:
                    # print neuro_space
...
def inference(inputpoints): #assume inputpoints is a list of lists/tuples; datapoints is an OrderedDict，這個沒包含vector
    #print "infer"
    neuron_number = 0
