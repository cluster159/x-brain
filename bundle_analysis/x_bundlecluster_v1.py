...
try:
    os.mkdir("xbundle_22500_sig30_50_cluster_0p4")
except:
    print "exist"

for dirPath, dirNames, fileNames in os.walk("xbrain_result_long_50_sig30_22500/"):
    for f in fileNames:
...
def get_data(neuron, Time, Value,length):
    name="result_"+neuron+"_length_"+length
    with open("xbrain_result_long_50_sig30_22500/" +name, "rt") as f_neuron:
        for line in f_neuron:
            ...
            #     2-2. 若在字典裡面，取min值，計次+1，將相似的cluster編號全部換成最小的data編號
             #---------------------------
            if (p >= (0.4*100000) and neuron!=neuron_name):
...
for neuron in url:
    ...
    neuron=grouping[1]+"_"+grouping[2]
    cluster[neuron]=i
    i = i + 1
...
for neuron in url:
    ...
    neuron=grouping[1]+"_"+grouping[2]
    length=grouping[4]
    get_data(neuron, Time, Value,length)
    i = i + 1
...
for j in xrange(len(count_list)):
    if count_list[j] >= 200:
        with open("xbundle_22500_sig30_50_cluster_0p4/static_cluster_200up.txt", "at") as fp:
            fp.writelines("cluster_" + str(j) + '\n')
    elif count_list[j] >= 100:
        with open("xbundle_22500_sig30_50_cluster_0p4/static_cluster_100up.txt", "at") as fp:
            fp.writelines("cluster_" + str(j) + '\n')
    elif count_list[j] >= 50:
        with open("xbundle_22500_sig30_50_cluster_0p4/static_cluster_50up.txt", "at") as fp:
            fp.writelines("cluster_" + str(j) + '\n')
    elif count_list[j] >= 10:
        with open("xbundle_22500_sig30_50_cluster_0p4/static_cluster_10up.txt", "at") as fp:
            fp.writelines("cluster_" + str(j) + '\n')
    elif count_list[j] >= 3:
        with open("xbundle_22500_sig30_50_cluster_0p4/static_cluster_3up.txt", "at") as fp:
            fp.writelines("cluster_" + str(j) + '\n')
    elif count_list[j] >= 1:
        with open("xbundle_22500_sig30_50_cluster_0p4/static_cluster_1up.txt", "at") as fp:
            fp.writelines("cluster_" + str(j) + '\n')

##寫出每個cluster的成員--
# for n in cluster:
#     with open ("xbundle_22500_cluster_0p4/cluster_"+str(cluster[n])+".txt","at") as fp:
#         fp.writelines(str(n)+ "\n")
...
for j in xrange(len(assembly_collection)):
    with open ("xbundle_22500_sig30_50_cluster_0p4/cluster_"+str(assembly_index[j])+"_correlation_map.txt","wt") as fp:
