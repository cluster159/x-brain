...
url=["static_cluster_200up.txt","static_cluster_100up.txt","static_cluster_10up.txt","static_cluster_3up.txt"]
warping_list=[]
...
for brain_index in xrange(len(warping_list)):
    ...
    neuron_collection = []
    for i in url:
        with open("xbundle_22500_sig20_50_cluster_0p4/" + i, "rt") as fp:
            for j in fp:
                if j[-1] == '\n':
    ...
    for i in cluster_list:
        neuron_list = []
        try:
            with open("xbundle_22500_sig20_50_cluster_0p4/sorting_" + i + ".txt", "rt") as fp:
                for j in fp:
                    if j[-1] == '\n':
                        j = j[:-1]
    ...
    for i in neuron_collection:
        ...
        for neuron in i:
            check_group=neuron
            neuron_index=check_group.split("_")
            if int(neuron_index[0])==brain_index:
    ...
    try:
        os.mkdir("xbrain_22500_bundlefig1/single_brain")
    except:
        pass
    re_swc = re.compile(
    ...
    for iii in xrange(3):
        for jjj in xrange(3):
            ...
            plt.savefig("xbrain_22500_bundlefig1/single_brain/sig20_"+warping_list[brain_index]+"_" + str(iii) + "_" + str(jjj) + ".png", dpi=100)
    plt.close()
    # plt.show()
